...
function renderCountdownSetup() {
  ...
  content.innerHTML = stage("Đếm ngược", "Chọn mốc nhanh hoặc tự đặt thời gian", `<div class="quick-times">${[5,10,15,30,60,90,120].map((minutes) => `<button data-quick="${minutes}" type="button">${minutes} phút</button>`).join("")}</div><div class="custom-countdown"><label>Số giờ<select id="custom-hours">${numberOptions(0, 12, 0)}</select></label><label>Số phút<select id="custom-minutes">${numberOptions(0, 59, 0)}</select></label><button id="custom-countdown" class="btn" type="button">Chọn thời gian</button></div><p id="duration-error" class="err" role="alert"></p>`);
...
function openCountdown(duration) {
  if (!Number.isFinite(duration) || duration <= 0 || duration > 12 * 3600000) {
    $("duration-error").textContent = "Chọn thời gian lớn hơn 0 và không quá 12 giờ.";
    return;
  }
  clearTimer();
  countdown = { initial: duration, remaining: duration, startedAt: Date.now(), running: true };
  renderCountdownRun();
}
function renderCountdownRun(message = "") {
  clearTimer();
  const remaining = countdownMs();
  content.innerHTML = stage("Đếm ngược", "Giờ : Phút : Giây", `<output id="clock-display" class="clock-display">${countdownText(remaining)}</output><p id="clock-message" class="clock-message" role="status">${message}</p><div class="clock-actions countdown-actions"><button id="countdown-stop" class="clock-action pause" type="button" ${remaining === 0 ? "disabled" : ""}>${countdown.running ? "Dừng" : "Tiếp tục"}</button><button id="countdown-reset" class="clock-action reset" type="button" ${countdown.running ? "disabled" : ""}>Đặt lại</button><button id="countdown-cancel" class="clock-action cancel" type="button" ${countdown.running ? "disabled" : ""}>Hủy</button></div>`);
  $("countdown-stop").onclick = () => {
    if (countdown.running) {
      countdown.remaining = countdownMs();
      countdown.running = false;
    } else if (countdown.remaining > 0) {
      countdown.startedAt = Date.now();
      countdown.running = true;
    }
    renderCountdownRun(countdown.remaining === 0 ? "Đã hết giờ." : "");
  };
  $("countdown-reset").onclick = () => {
    if (countdown.running) return;
    countdown.remaining = countdown.initial;
    countdown.startedAt = 0;
    renderCountdownRun();
  };
  $("countdown-cancel").onclick = () => {
    if (countdown.running) return;
    countdown = { initial: 0, remaining: 0, startedAt: 0, running: false };
    renderCountdownSetup();
  };
  if (countdown.running) timer = setInterval(() => {
    const remainingNow = countdownMs();
    $("clock-display").textContent = countdownText(remainingNow);
    if (remainingNow === 0) {
      countdown.running = false;
      countdown.remaining = 0;
      countdown.startedAt = 0;
      renderCountdownRun("Đã hết giờ.");
    }
  }, 200);
}
